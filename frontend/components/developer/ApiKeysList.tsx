...
export function ApiKeysList({
  ...
  const statusBadge = (status: ApiKey['status']) => {
    ...
    if (status === 'active')
      return `${base} bg-emerald-500/10 border border-emerald-500/30 text-emerald-400`;
    if (status === 'expired')
      return `${base} bg-amber-500/10 border border-amber-500/30 text-amber-400`;
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <span className={statusBadge(key.status)}>
                      {key.status}
                    </span>
                  </div>
                </div>
