...
export default function AuditLogsPage() {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Action', 'Entity', 'Entity ID', 'User ID', 'IP Address', 'Date'];
    const csvContent = [
      headers.join(','),
      ...logs.data.map(log => [
        log.action,
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-blue-200/60 mt-1">Track all system activities and security events across the protocol.</p>
          </div>
        </div>
  ...
  return (
    <div className="bg-white/5 backdrop-blur-sm rounded-3xl p-6 border border-white/10 flex flex-col justify-between group hover:border-white/20 transition-all duration-300">
      <div className="flex items-start justify-between">
        ...
        <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider border ${colorMap[color]}`}>
          {trend}
        </span>
      </div>
      <div className="mt-4">
        <p className="text-sm font-medium text-blue-200/60 uppercase tracking-wider">{title}</p>
