...
- **High Error Rate**:
  - _Threshold_: > 1% over 5 minutes.
  - _Severity_: High (Page on-call).
- **High Latency (API)**:
  - _Threshold_: p95 latency > 500ms over 5 minutes.
  - _Severity_: Medium (Ping Slack channel).
- **CPU Saturation (Service)**:
  - _Threshold_: > 80% usage for 10 minutes.
  - _Severity_: Low/Medium (Auto-scaling group should handle, alert if it fails).
- **Database Connection Pool Exhaustion**:
  - _Threshold_: Active connections > 90% of max allowed.
  - _Severity_: High.
...
_Refer to the [Troubleshooting Guide](troubleshooting-guide.md) for in-depth resolution steps._
