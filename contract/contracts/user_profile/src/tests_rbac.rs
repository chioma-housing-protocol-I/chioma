...
//! upgrade proposal lifecycle) reject calls missing the required
//! `require_auth`, and that admin-only actions are restricted to the
//! configured admin.
...
#[test]
#[should_panic(expected = "Error(Contract, #6)")]
fn test_unverify_profile_fails_if_not_admin() {
    let env = Env::default();
    let client = create_contract(&env);
